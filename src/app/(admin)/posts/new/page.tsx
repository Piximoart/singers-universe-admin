  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Nový post</h1>
          <p className="text-sub text-sm">Přidejte příspěvek do feedu</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        <div className="bg-s1 border border-border rounded-lg p-5 space-y-4">
          ...
          <FormField type="text" label="Titulek" name="title" value={form.title} onChange={(v) => set("title", v)} placeholder="Oznámení nového singlu..." />

          <FormField type="textarea" label="Text" name="body" value={form.body} onChange={(v) => set("body", v)} rows={5} placeholder="Obsah příspěvku..." />

          <FormField type="select" label="Propojit s trackem (volitelné)" name="track_id" value={form.track_id} onChange={(v) => set("track_id", v)} options={tracks} />
        </div>

        <div className="bg-s1 border border-border rounded-lg p-5">
          <h2 className="text-sm font-semibold text-white mb-4">Médium (volitelné)</h2>
          <ImageUpload
            label="Obrázek / cover"
            currentUrl={form.media_url}
            onUpload={(url) => set("media_url", url)}
            uploadPath={`posts/${Date.now()}.jpg`}
          />
        </div>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={saving} className="bg-lime text-bg text-sm font-semibold px-6 py-2.5 rounded-md hover:bg-white transition-colors disabled:opacity-50">
            {saving ? "Ukládám..." : "Publikovat post"}
          </button>
